...
func postMsgWebhooks(hookURLs []string, title, text string, userName, iconEmoji string) ERR {
	for _, hurl := range hookURLs {
		if err := postMsgWebhook(hurl, title, text, userName, iconEmoji); err != NIL {
			return err
		}
	}
...
func postMsgWebhook(hookURL string, title, text string, userName, iconEmoji string) ERR {
	return postMsgWebhookWithChannel(hookURL, "", title, text, userName, iconEmoji)
}

func postMsgWebhookWithChannels(hookURL string, channelIDs []string, title, text string, userName, iconEmoji string) ERR {
	for _, chID := range channelIDs {
		if err := postMsgWebhookWithChannel(hookURL, chID, title, text, userName, iconEmoji); err != NIL {
			return err
		}
	}
...
func postMsgWebhookWithChannel(hookURL string, channelID, title, text string, userName, iconEmoji string) ERR {
